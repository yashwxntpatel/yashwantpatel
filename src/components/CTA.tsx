...
const marqueeText = "Let's build something that actually works ·";
...
export const CTA = memo(() => (
  ...
  >
    <div
      className="absolute top-0 left-0 right-0 overflow-hidden border-y border-border/40 py-8 md:py-12"
      aria-hidden="true"
    >
      <div className="flex w-max animate-marquee whitespace-nowrap">
        {Array.from({ length: 4 }).map((_, index) => (
          <span
            key={index}
            className="font-serif italic text-6xl md:text-8xl text-foreground/10 px-8"
          >
            {marqueeText}
          </span>
        ))}
    ...
    <div className="container-luxe pt-40 md:pt-56">
      <div className="relative mx-auto max-w-4xl text-center">
        <div
          className="absolute inset-0 -z-10 bg-gradient-radial blur-3xl opacity-60"
          aria-hidden="true"
        />
        ...
        >
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-xl mx-auto mb-12">
            Brief me on your project. I reply within 24 hours with timing,
            scope, and a straight answer on whether I'm the right fit.
          </p>
          ...
          <div className="grid sm:grid-cols-3 gap-6 max-w-2xl mx-auto pt-12 border-t border-border/60 text-sm">
            <div>
              ...
              <a
                href="mailto:yashwantpatel.in@gmail.com"
    ...
    <footer className="container-luxe mt-32 pt-10 border-t border-border/40 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-muted-foreground">
      <div>
        © {new Date().getFullYear()} Yashwant Patel. All Rights Reserved.
      </div>
