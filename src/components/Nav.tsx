      ...
      <div className="container-luxe">
        <nav className={`flex items-center justify-between rounded-full px-5 py-3 transition-all duration-500 ${scrolled ? "glass-strong" : ""}`}>
          <a href="#top" className="flex items-center gap-2 group">
            <span className="relative flex h-8 w-8 items-center justify-center rounded-full bg-gradient-amber text-accent-foreground font-bold text-sm shadow-glow">
              Y
            </span>
          ...
          <ul className="hidden md:flex items-center gap-1">
            {links.map((l) => (
              <li key={l.href}>
                <a href={l.href} className="relative px-4 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors rounded-full hover:bg-foreground/5">
                  {l.label}
                </a>
              </li>
            ))}
          ...
          <button className="md:hidden p-2 -mr-2" aria-label="Toggle menu" onClick={toggleMenu}>
            <div className="w-5 flex flex-col gap-1.5">
              <motion.span animate={{ rotate: open ? 45 : 0, y: open ? 6 : 0 }} className="h-px bg-foreground origin-center" />
              <motion.span animate={{ opacity: open ? 0 : 1 }} className="h-px bg-foreground" />
              <motion.span animate={{ rotate: open ? -45 : 0, y: open ? -6 : 0 }} className="h-px bg-foreground origin-center" />
            </div>
          </button>
        </nav>
        ...
        <AnimatePresence>
          {open && (
            ...
            >
              {links.map((l) => (
                <a key={l.href} href={l.href} onClick={closeMenu} className="px-4 py-3 rounded-xl hover:bg-foreground/5 text-sm">
                  {l.label}
                </a>
              ))}
            </motion.div>
